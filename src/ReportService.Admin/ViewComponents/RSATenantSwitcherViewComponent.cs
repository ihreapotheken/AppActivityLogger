...
public sealed class RSATenantSwitcherViewComponent : ViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync()
    {
        ...
        var displayByClient = (await _catalog.ListClientsAsync(includeArchived: false, ct).ConfigureAwait(false))
            .ToDictionary(c => c.Slug, c => c.DisplayName, StringComparer.Ordinal);

        var groups = apps
            .GroupBy(a => a.ClientSlug, StringComparer.Ordinal)
            .OrderBy(g => g.Key, StringComparer.Ordinal)
            .Select(g => new RSATenantSwitcherGroup(
                ClientSlug: g.Key,
                ClientDisplay: displayByClient.TryGetValue(g.Key, out var d) ? d : g.Key,
                Apps: g.OrderBy(a => a.Slug, StringComparer.Ordinal).Select(a => new RSATenantSwitcherApp(
                    AppSlug: a.Slug,
        ...
        var vm = new RSATenantSwitcherVM(
            ...
            HasSelection: currentApp is not null,
            ReturnUrl: returnUrl,
            Groups: groups);
        return View(vm);
    }
...
public sealed record RSATenantSwitcherGroup(string ClientSlug, string ClientDisplay, IReadOnlyList<RSATenantSwitcherApp> Apps);
