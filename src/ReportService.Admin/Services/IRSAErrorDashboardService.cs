...
public interface IRSAErrorDashboardService
{
    ...
    /// when null it falls back to the rolling last 7 days, bucketed daily. <paramref name="clientId"/>
    /// / <paramref name="appId"/> scope every tile + rollup to one tenant/app (null = all apps),
    /// matching the global (client, app) selection.</summary>
    RSAErrorDashboardVM Build(string? platform = null, RSAErrorRateWindow? rateWindow = null, string? clientId = null, string? appId = null);
    ...
    /// </summary>
    IReadOnlyList<RSATrendingIssueVM> BuildTrending(int recentDays = 7, int limit = 5, string? clientId = null, string? appId = null);
}
