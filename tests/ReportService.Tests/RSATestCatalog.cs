...
/// validation — every app / environment / client is considered valid, so batches with no explicit
/// attribution (which resolve to the default tenant) pass the validator exactly as they did before
/// the tenancy axes existed. Tests that specifically assert tenancy validation/isolation use the
/// real <see cref="RSCSqliteCatalog"/> instead.
/// </summary>
internal sealed class RSATestCatalog : RSCICatalog
{
    ...
    public bool IsValidEnvironment(string clientSlug, string appSlug, string environment) => true;
    ...
    public Task<RSCAppRecord> CreateAppAsync(string clientSlug, string appSlug, string displayName, IReadOnlyList<string> environments, CancellationToken ct) =>
        Task.FromResult(new RSCAppRecord("app_test", clientSlug, appSlug, displayName, DateTimeOffset.UtcNow, null, environments));
    ...
    public Task<bool> AddEnvironmentAsync(string clientSlug, string appSlug, string environment, CancellationToken ct) => Task.FromResult(true);
    public Task<bool> RemoveEnvironmentAsync(string clientSlug, string appSlug, string environment, CancellationToken ct) => Task.FromResult(true);
